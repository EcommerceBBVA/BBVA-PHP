...
abstract class OpenpayApiResourceBase
{
    ...
    protected function __construct($resourceName, $params = array()) {
        ...
        if (!is_array($params)) {
            throw new OpenpayApiError("Invalid parameter type detected when instantiating an Bancomer resource (passed '".gettype($params)."', array expected)");
        }
    ...
    protected static function getInstance($resourceName, $props = null) {
        ...
        if (!class_exists($resourceName)) {
            throw new OpenpayApiError("Invalid Bancomer resource type (class resource '".$resourceName."' is invalid)");
        }
        if (is_string($props)) {
...
// 		$resourceName = $this->getResourceName($name);
    ...
    protected function refreshData($data) {
        ...
        if (!is_array($data)) {
            throw new OpenpayApiError("Invalid data received for processing, cannot update the Bancomer resource");
        }
